...
def load_yaml_config(path: str) -> dict:
    ...
    NOTE: This is a hand-rolled parser for the simple key: value format used in
    config.yaml.  It does NOT support multi-line values, YAML anchors, or values
    that contain a '#' character (those are treated as inline comments).  If the
    config ever needs those features, swap this function for a PyYAML load.
    """
    overrides = {}
    with open(path) as f:
        for line in f:
...
def parse_args() -> argparse.Namespace:
    ...
    # Paths
    p.add_argument("--raw-data-dir", default="data/raw",
                   help="Root directory containing mit-bih/, eeg-motor/, ppg-dalia/ sub-folders")
    p.add_argument("--processed-dir", default="data/processed",
                   help="Directory for preprocessed HDF5 outputs")
...
def build_config(args: argparse.Namespace):
    """Convert CLI args to a UnifiedRunConfig, with YAML filling gaps only.

    Priority (highest → lowest):
        1. Explicit CLI flags  (e.g. --epochs 30)
        2. YAML values         (e.g. epochs: 100 in config.yaml)
        3. argparse defaults

    BUG 3 FIX: the original code unconditionally overwrote every arg with the
    YAML value (all three branches of the if/elif/else ultimately called
    setattr), so YAML always won.  The fix collects every flag the user actually
    typed on the command line and only applies YAML values for the rest.
    """
